    ...
    try {
        ...
        if (getNetlifyKey === getValidationKey) {
                ...
                const signIntoThinkific = (user) => {
                    ...
                    let url = `https://${process.env.THINKIFIC_SUB_DOMAIN}.thinkific.com/api/sso/v2/sso/jwt?jwt=${token}&course_id=${process.env.COURSE_ID}`;
                ...
                const createOrEnrolStudent = async (userData) => {
                    try {
                        ...
                        if (userExistData.error === "Record not found") {
                            ...
                            //const redirectUrl = signIntoThinkific(userData); JUST FOR TESTING
                            //await sendRedirecctLinkToWebhook(userData.email, redirectUrl);
                        } else {
                            //user alreeady exist
                            return {
